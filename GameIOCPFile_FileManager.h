	...
	namespace IOCP
	{
		namespace File
		{
			...
			public:
				...
				void SetOnRead(std::function<void(const int32_t result, const DWORD bytesTransferred, const DWORD bytesToTransfer, const uint8_t* data)>);
			...
			private:
				...
				HANDLE _iocpHandle;
				std::function<void(const int32_t result, const DWORD bytesTransferred, const DWORD bytesToTransfer, const uint8_t* data)> _onRead;
			};
		}
	}
}